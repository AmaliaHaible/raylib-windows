...
void DrawTextureTiled(Texture2D texture, RL_Rectangle source, RL_Rectangle dest, Vector2 origin, float rotation, float scale, Color tint);
...
int main(void)
{
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        BeginDrawing();
            ...
            // Draw the tiled area
            DrawTextureTiled(texPattern, recPattern[activePattern], (RL_Rectangle){(float)OPT_WIDTH+MARGIN_SIZE, (float)MARGIN_SIZE, GetScreenWidth() - OPT_WIDTH - 2.0f*MARGIN_SIZE, GetScreenHeight() - 2.0f*MARGIN_SIZE},
...
void DrawTextureTiled(Texture2D texture, RL_Rectangle source, RL_Rectangle dest, Vector2 origin, float rotation, float scale, Color tint)
{
    ...
    if ((dest.width < tileWidth) && (dest.height < tileHeight))
    {
        ...
        DrawTexturePro(texture, (RL_Rectangle){source.x, source.y, ((float)dest.width/tileWidth)*source.width, ((float)dest.height/tileHeight)*source.height},
    ...
    else if (dest.width <= tileWidth)
    {
        ...
        {
            DrawTexturePro(texture, (RL_Rectangle){source.x, source.y, ((float)dest.width/tileWidth)*source.width, source.height}, (RL_Rectangle){dest.x, dest.y + dy, dest.width, (float)tileHeight}, origin, rotation, tint);
        }

        // Fit last tile
        if (dy < dest.height)
        {
            DrawTexturePro(texture, (RL_Rectangle){source.x, source.y, ((float)dest.width/tileWidth)*source.width, ((float)(dest.height - dy)/tileHeight)*source.height},
    ...
    else if (dest.height <= tileHeight)
    {
        ...
        {
            DrawTexturePro(texture, (RL_Rectangle){source.x, source.y, source.width, ((float)dest.height/tileHeight)*source.height}, (RL_Rectangle){dest.x + dx, dest.y, (float)tileWidth, dest.height}, origin, rotation, tint);
        }

        // Fit last tile
        if (dx < dest.width)
        {
            DrawTexturePro(texture, (RL_Rectangle){source.x, source.y, ((float)(dest.width - dx)/tileWidth)*source.width, ((float)dest.height/tileHeight)*source.height},
    ...
    else
    {
        ...
        for (;dx+tileWidth < dest.width; dx += tileWidth)
        {
            ...
            for (;dy+tileHeight < dest.height; dy += tileHeight)
            {
                DrawTexturePro(texture, source, (RL_Rectangle){dest.x + dx, dest.y + dy, (float)tileWidth, (float)tileHeight}, origin, rotation, tint);
            }

            if (dy < dest.height)
            {
                DrawTexturePro(texture, (RL_Rectangle){source.x, source.y, source.width, ((float)(dest.height - dy)/tileHeight)*source.height},
        ...
        if (dx < dest.width)
        {
            ...
            for (;dy+tileHeight < dest.height; dy += tileHeight)
            {
                DrawTexturePro(texture, (RL_Rectangle){source.x, source.y, ((float)(dest.width - dx)/tileWidth)*source.width, source.height},
            ...
            if (dy < dest.height)
            {
                DrawTexturePro(texture, (RL_Rectangle){source.x, source.y, ((float)(dest.width - dx)/tileWidth)*source.width, ((float)(dest.height - dy)/tileHeight)*source.height},
