    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        BeginDrawing();
            ...
            BeginBlendMode(blending);
                ...
                for (int i = 0; i < MAX_PARTICLES; i++)
                {
                    if (mouseTail[i].active) DrawTexturePro(smoke, (RL_Rectangle){ 0.0f, 0.0f, (float)smoke.width, (float)smoke.height },
            ...
            RL_DrawText("PRESS SPACE to CHANGE BLENDING MODE", 180, 20, 20, BLACK);

            if (blending == BLEND_ALPHA) RL_DrawText("ALPHA BLENDING", 290, screenHeight - 40, 20, BLACK);
            else RL_DrawText("ADDITIVE BLENDING", 280, screenHeight - 40, 20, RAYWHITE);
